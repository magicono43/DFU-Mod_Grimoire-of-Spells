...
namespace GrimoireofSpells
{
    ...
    /// </summary>
    public class TransmuteBody : IncumbentEffect
    {
        public static readonly string effectKey = "Transmute-Body";

        // Str, Agi, End, Spe
        protected int[] magStats = { 0, 0, 0, 0, 0, 0, 0, 0 };
        protected bool[] curseChecks = { true, false, false, true, true, false, true, false };
        ...
        public override string GroupName => "Transmute Body";
        ...
        private TextFile.Token[] GetSpellMakerDescription() // Will definitely want to change the descriptions after testing and such, place-holder for now.
        {
            return DaggerfallUnity.Instance.TextProvider.CreateTokens(
                ...
                GroupName,
                "Curses the caster's Strength, Agility, Endurance, and Speed,",
        ...
        protected override void AddState(IncumbentEffect incumbent)
        {
            ...
            if (lastMagnitudeIncreaseAmount > 0)
            {
                (incumbent as TransmuteBody).IncreaseMagnitude(lastMagnitudeIncreaseAmount);
                ShowPlayerCursed();
            }
        }
        ...
        public override void HealAttributeDamage(DFCareer.Stats stat, int amount)
        {
            ...
            if (!(stat == DFCareer.Stats.Strength || stat == DFCareer.Stats.Agility || stat == DFCareer.Stats.Endurance || stat == DFCareer.Stats.Speed))
                return;
            ...
            if (DecreaseMagnitude(amount, stat) == 0)
            {
                ...
                if (magStats[(int)DFCareer.Stats.Strength] == 0 && magStats[(int)DFCareer.Stats.Agility] == 0 && magStats[(int)DFCareer.Stats.Endurance] == 0 && magStats[(int)DFCareer.Stats.Speed] == 0)
        ...
        public void IncreaseMagnitude(int amount)
        {
            ...
            magStats[(int)DFCareer.Stats.Strength] += amount * 30;
            magStats[(int)DFCareer.Stats.Agility] += amount * 30;
            magStats[(int)DFCareer.Stats.Endurance] += amount * 30;
            magStats[(int)DFCareer.Stats.Speed] += amount * 30;
            ...
            SetStatMod(DFCareer.Stats.Strength, (int)Mathf.Ceil(-1 * (magStats[(int)DFCareer.Stats.Strength] / 30)));
            SetStatMod(DFCareer.Stats.Agility, (int)Mathf.Ceil(-1 * (magStats[(int)DFCareer.Stats.Agility] / 30)));
            SetStatMod(DFCareer.Stats.Endurance, (int)Mathf.Ceil(-1 * (magStats[(int)DFCareer.Stats.Endurance] / 30)));
            SetStatMod(DFCareer.Stats.Speed, (int)Mathf.Ceil(-1 * (magStats[(int)DFCareer.Stats.Speed] / 30)));
        }
